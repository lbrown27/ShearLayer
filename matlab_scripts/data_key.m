function indices = data_key(input)
indices = zeros(1,length(input));
for i = 1:length(input)
names = ["nodenumber","x","y","pressure",...
    "pressure_coefficient","dynamic_pressure","absolute_pressure",...
    "total_pressure","rel_total-pressure","density","density_all",...
    "velocity_magnitude","u","v","stream_fn","radial_velocity",...
    "tangential_velocity","mach_number","rel_velocity_magnitude",...
    "relative_x_velocity","relative_y_velocity","rel_tangential_velocity",...
    "rel_mach_number","mesh_x_velocity","mesh_y_velocity","velocity_angle",...
    "relative_velocity_angle","vorticity_mag","cell_reynolds_number","uref",...
    "q_criterion"]; % "-"'s are changed to "_"'s in order to create valid field names.
for j = 1:31
    if strcmp(names(j), input(i))
        indices(i) = j;
    end
end
end
% raw-q-criterion:32
% lambda2-criterion:33
% temperature:34
% total-temperature:35
% enthalpy:36
% rel-total-temperature:37
% rothalpy:38
% wall-temperature:39
% wall-adjacent-temperature:40
% reference-temperature-at-y+:41
% wall-temp-thin:42
% total-enthalpy:43
% total-enthalpy-deviation:44
% entropy:45
% total-energy:46
% internal-energy:47
% turb-kinetic-energy:48
% turb-intensity:49
% turb-diss-rate:50
% specific-diss-rate:51
% production-of-k:52
% viscosity-turb:53
% viscosity-eff:54
% viscosity-ratio:55
% thermal-conductivity-eff:56
% prandtl-number-eff:57
% y-star:58
% y-plus:59
% turb-reynolds-number-rey:60
% viscosity-lam:61
% thermal-conductivity-lam:62
% specific-heat-cp:63
% gamma:64
% gas-constant:65
% prandtl-number-lam:66
% sound-speed:67
% wall-shear:68
% x-wall-shear:69
% y-wall-shear:70
% skin-friction-coef:71
% heat-flux:72
% heat-transfer-coef:73
% heat-transfer-coef-wall-adj:74
% heat-transfer-coef-yplus:75
% heat-transfer-coef-wall:76
% nusselt-number:77
% stanton-number:78
% cell-partition-active:79
% cell-partition-stored:80
% cell-id:81
% cell-element-type:82
% cell-type:83
% cell-zone,partition-neighbors,     cell-weight,    x-coordinate,    y-coordinate,axial-coordinate,radial-coordinate,face-area-magnitude,     x-face-area,     y-face-area,     cell-volume,orthogonal-quality,cell-equiangle-skew,cell-equivolume-skew, face-handedness,mark-poor-elements,interface-overlap-fraction,cell-wall-distance,cell-refine-level,boundary-cell-dist,boundary-normal-dist,boundary-volume-dist,cell-volume-change,       time-step, strain-rate-mag,  dx-velocity-dx,  dy-velocity-dx,           dt-dx,  dx-velocity-dy,  dy-velocity-dy,           dt-dy
% 
% 


%% Key
% nodenumber: 1
% x-coordinate: 2
% y-coordinate: 3
% pressure: 4
% pressure-coefficient:5
% dynamic-pressure:6
% absolute-pressure:7
% total-pressure:8
% rel-total-pressure:9
% density:10
% density-all:11
% velocity-magnitude:12
% x-velocity:13
% y-velocity:14
% stream-fn:15
% radial-velocity:16
% tangential-velocity:17
% mach-number:18
% rel-velocity-magnitude:19
% relative-x-velocity:20
% relative-y-velocity:21
% rel-tangential-velocity:22
% rel-mach-number:23
% mesh-x-velocity:24
% mesh-y-velocity:25
% velocity-angle:26
% relative-velocity-angle:27
% vorticity-mag:28
% cell-reynolds-number:29
% uref:30
% q-criterion:31
% raw-q-criterion:32
% lambda2-criterion:33
% temperature:34
% total-temperature:35
% enthalpy:36
% rel-total-temperature:37
% rothalpy:38
% wall-temperature:39
% wall-adjacent-temperature:40
% reference-temperature-at-y+:41
% wall-temp-thin:42
%total-enthalpy:43
%total-enthalpy-deviation:44
%entropy:45
%total-energy:46
%internal-energy:47
%turb-kinetic-energy:48
%turb-intensity:49
%turb-diss-rate:50
%specific-diss-rate:51
%production-of-k:52
%viscosity-turb:53
%viscosity-eff:54
%viscosity-ratio:55
%thermal-conductivity-eff:56
%prandtl-number-eff:57
%y-star:58
%y-plus:59
%turb-reynolds-number-rey:60
%viscosity-lam:61
%thermal-conductivity-lam:62
%specific-heat-cp:63
%gamma:64
%gas-constant:65
%prandtl-number-lam:66
%sound-speed:67
%wall-shear:68
%x-wall-shear:69
%y-wall-shear:70
%skin-friction-coef:71
%heat-flux:72
%heat-transfer-coef:73
%heat-transfer-coef-wall-adj:74
%heat-transfer-coef-yplus:75
% heat-transfer-coef-wall:76
% nusselt-number:77
% stanton-number:78
% cell-partition-active:79
% cell-partition-stored:80
% cell-id:81
% cell-element-type:82
% cell-type:83
%cell-zone,partition-neighbors,     cell-weight,    x-coordinate,    y-coordinate,axial-coordinate,radial-coordinate,face-area-magnitude,     x-face-area,     y-face-area,     cell-volume,orthogonal-quality,cell-equiangle-skew,cell-equivolume-skew, face-handedness,mark-poor-elements,interface-overlap-fraction,cell-wall-distance,cell-refine-level,boundary-cell-dist,boundary-normal-dist,boundary-volume-dist,cell-volume-change,       time-step, strain-rate-mag,  dx-velocity-dx,  dy-velocity-dx,           dt-dx,  dx-velocity-dy,  dy-velocity-dy,           dt-dy